%% Ptychography for HFL40 30 MHz transducer, use tungsten target data.
function [Output, OutputImage, RefImage] = Ptychography30MHzTungsten(inputParams)
%% 输入参数
...
global data3;
imSeqLowRes = data3;
[depthPts, radiusPts, number] = size(data3);
...
%% 图像处理 & 校正
% Output Image
OutputImage = max(0, imresize(abs(objectRecover), [depthPts, radiusPts]));
OutputImage = OutputImage(:, 100:760);
OutputImage = dynamic(OutputImage, 40, 0, 'wusi', 3);
...
for i = 1 : 4
    ...
    Output.resR = Output.resR + calResIR(OutputImage(z(i), range1), spsize, 0.9 * max(OutputImage(z(i), range1)), 12);
    range2 = z(i)-15:z(i)+15;
    Output.resH = Output.resH + calResIR(OutputImage(range2, x(i)), pixelH, 0.9 * max(OutputImage(range2, x(i))), 12);
end
...
Output.contrast = 0;
for i = 1 : 4
    Output.contrast = Output.contrast + calCNR(OutputImage(z(i)-12:z(i)+12, x(i)-12:x(i)+12), 1);
end
